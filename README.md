...
**Проблема M×N.** До MCP каждый host решал интеграции по-своему — ChatGPT через plugins (потом Actions в GPTs), Claude и OpenAI API через function calling, IDE через собственные расширения. В итоге: каждое приложение с LLM (Cursor, Claude Desktop, свой агент) × каждый внешний сервис (Slack, GitHub, Postgres…) = квадратичная интеграционная боль. Каждое приложение пишет свои адаптеры под каждый сервис заново.
...
Шестиугольник MCP в середине — условность: никакого центрального прокси нет, MCP — это **общий протокол**, на котором каждая сторона говорит один раз. Каждый клиент реализует MCP-поддержку один раз и получает все серверы; каждый сервер пишется один раз и его видят все клиенты.

**MCP — универсальный коннектор:** свести M×N к M+N. Сервис пишет **один** MCP-сервер — и его получают все клиенты. Клиент учится говорить MCP — и получает всю экосистему серверов.
...
Что популярный MCP-gateway теряет при проксировании. Опирается на §3 (capability negotiation, симметрия) и на `examples/08-sampling/` — читается после основной прогрессии.
