...
- **Битый JSON** (`{broken`) — JSON-RPC error под капотом не уходит (у невалидного сообщения нет `id`, отвечать некому). Сервер логирует warning и шлёт `notifications/message` с `level: "error"`, сам продолжает работать.
