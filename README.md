...
9. [Notifications](#9-notifications) → `examples/06-notifications/`
...
> **Статус:** готовы главы 1–10; 11–15 и оба appendix — в работе, содержимое меняется.
...
Server→client канал целиком: `progress` для долгих tool call'ов, `logging/message` для session-scoped логов, `list_changed` для инвалидации каталогов, `subscribe`/`updated` для подписок на ресурсы. Первая глава, где сервер сам пишет в клиент вне контекста request'а.

## 10. Cancellation

📁 [`examples/07-cancellation/`](examples/07-cancellation/)

Отмена tool call через `notifications/cancelled`. Кооперативная модель (cancel поднимается на ближайшем `await`), три race conditions, встроенных в дизайн, и главная мысль: **cancel ≠ undo** — try/finally для уборки, идемпотентность для необратимых шагов. В конце — короткая вводная в `tasks/cancel` (experimental, разбирается в appendix/a2a).
